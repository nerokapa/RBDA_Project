...
def get_tweets_statistic(tweets, movie_oa):
    ...
    assert movie_oa[2] == tweets[0].author.screen_name
    ...
    return offical_account_statistic
...
def load_writed_json(f):
    ...
    with open(f) as json_file:
        for row in json_file:
            import pdb; pdb.set_trace()
            collected.add(json.loads(row)['oa_screen_name'])
    return collected
...
if __name__ == '__main__':
    ...
    for id_name_account in movie_offical_accounts:
        ping_pong += 1
        if(id_name_account[2] in collceted_movie):
            continue
        tweets = get_all_tweets(id_name_account[2], auth=auths[ping_pong % 2])
        meta_account_info = get_tweets_statistic(tweets, id_name_account)
        to_write = json.dumps(meta_account_info)
        with open(output_file, 'a+') as foutput:
